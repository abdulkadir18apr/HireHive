...
import { getRecruiter } from "../context/apicalls";
...
export const Navbar = () => {
   ...
   const getRecruiterApi=async()=>{
    const res=await getRecruiter();
    if(res.success){
        setRecruiter(res.recruiter);
        setfullName(res.recruiter?.firstName);
    }
   }
   useEffect(()=>{
    getRecruiterApi();
                ...
                <div className="hidden lg:block">
                    <div className="flex items-center space-x-2">
                        ...
                        <img
                            ...
                            src={state.profile.profileImage?`http://localhost:8000/${state.profile?.profileImage?.slice(6)}`:"https://picsum.photos/200"}
                            alt="John Doe"
                        />
                        <button className="text-sm font-medium text-yellow-500 dark:text-yellow-100" onClick={logouthandler}>
                            Logout
                        </button>
